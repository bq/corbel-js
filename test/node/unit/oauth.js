...
describe('In OAUTH module', function () {
  ...
  describe('with authorization endpoint,', function () {
    ...
    it('do not allow a response type disctint to "code"', function () {
      ...
      expect(oauth.authorization.bind(corbelDriver.oauth.authorization, clientParams)).to.throw('Only "code" or "token" response type allowed')
    })

    it('fails with missing params', function () {
      expect(oauth.authorization).to.throw('Invalid client parameters')
    })
    ...
    it('fails with incomplete params', function () {
      ...
      expect(oauth.authorization.bind(corbelDriver.oauth.authorization, clientParams)).to.throw('Invalid client parameters')
    })
  })
  ...
  describe('with token endpoint', function () {
    ...
    it('do not allow a grant type disctint to "authorization_code"', function () {
      ...
      expect(oauth.token.bind(corbelDriver.oauth.token, clientParams)).to.throw('Only "authorization_code" grant type is allowed')
    })

    it('fails with missing params', function () {
      expect(oauth.token).to.throw('Invalid client parameters')
    })

    it('fails with incomplete params', function () {
      expect(oauth.token.bind(corbelDriver.oauth.token, undefined)).to.throw('Invalid client parameters')
    })
  })
