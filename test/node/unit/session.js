...
describe('In Session module', function() {
  ...
  it('stores data in localStorage when session is persistent', function() {
    ...
    expect(session.localStorage.getItem('accessToken')).to.be.equal('token');
  });
  ...
  describe('when destroy', function() {
    ...
    it('removes all data correctly', function() {
      ...
      session.destroy();
      expect(session.get('persistent')).to.be.equal(null);
      expect(session.get('accessToken')).to.be.equal(null);
      expect(session.get('accessToken')).to.be.equal(null);
